...
Visualizing the apparent trend, we for the 5 categories with the highest count, we a similar count as with the older counts. Note thought that two o the categories do not have counts until the start of 2019.
...
ggplot(pd_desc_counts, aes(x=week_median_date, y=week_count, color=PD_DESC)) +
  ...
  scale_x_date(breaks = '1 year') +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Top 5 categories from 2018, with smoothing showing trends\n similar to the overall arrest trend")
 
```
Put the counts for the 5 categories into separate variables.
...
Turn to question 3 about the precincts
...
ggplot(precinct_counts_fm_2018, aes(x=week_median_date, y=week_count, color=precinct)) +
  ...
  scale_x_date(breaks = '1 year') +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = "Two precinct from 2018, with smoothing showing trends\n similar to the overall arrest trend")
